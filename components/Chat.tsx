...
export default function Chat({ user, displayName }: ChatProps) {
  ...
  const handleMedicalDialogSubmit = async (payload: DiagnosticPayload) => {
    try {
      ...
      const diagnosticsData = {
        ...
        testName: currentWorkerResponse.testName,
        testId: currentWorkerResponse.testId, // Use AI worker's recommended testId (or testName as fallback)
        selectedTests: currentWorkerResponse.testId
          ? [currentWorkerResponse.testId]
          : payload.testSelection.selectedTests, // Use AI testId or fallback to selected tests
